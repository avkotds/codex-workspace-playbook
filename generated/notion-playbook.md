...
GitHub is canonical for this public playbook and its downloadable templates. Mirrors such as a Notion page should be generated from the repository, not edited as a competing source.
...
5. Choose how the work should be done and which worker is capable of doing it.
6. Delegate to a project-scoped execution thread with a bounded contract.
7. Inspect the returned evidence and verify the real result.
8. Archive completed work and preserve unresolved blockers.

#### How the manager chooses

The manager makes two separate routing decisions. First it chooses the execution interface—the safest, most reliable way to act. Only then does it choose the worker model and reasoning level.

1. **Read the relevant skill, runbook, or source of truth.** This explains how the workflow operates, including its rules and verification gates.
2. **Select the execution interface.** Prefer the strongest structured route that can reliably perform the task.
3. **Select the worker model and reasoning.** Match capability to ambiguity, consequence, and the shape of the work.
4. **Send a bounded contract.** State the outcome, scope, rules, done criteria, and expected return.
5. **Review evidence and verify.** Inspect targeted evidence, then check the actual artifact or runtime when the task requires it.

The execution interfaces have different jobs:

- **Skill or runbook:** instructions for how a workflow operates.
- **MCP, app connector, or API:** a structured action with explicit inputs and outputs.
- **CLI, SDK, project script, or SSH:** the authoritative command workflow for a project or system.
- **Browser or Chrome:** authenticated web-session, DOM, and rendered-web work.
- **Computer Use:** genuinely visual desktop work when no reliable structured route exists.

Use this reliability ladder:

**MCP/app/API → CLI/SDK/SSH → Browser/Chrome → Computer Use**

If a structured call fails, diagnose its **identity, authentication, permissions, schema, target, and callability**. Never silently fall back to Browser or Computer Use. A hybrid is often best: make the mutation through the structured interface, then use the UI only for visual verification when that adds useful evidence.

Beginner examples:

- **Notion schema or page content:** use Notion MCP.
- **Web app mutation:** use its API or CLI, then use Browser for visual verification when useful.
- **Windows GUI with no structured interface:** use Computer Use.

#### Why split the models

Start with general capability guidance: use the most capable reasoning model for the manager, because it must understand ambiguous requests, route them, and review evidence. Give bounded execution to the lowest-cost, fastest worker that can reliably complete it. Workers need more reasoning as ambiguity, consequence, or cross-system complexity increases—not simply because a task is long.
...
This is a dated example, not a permanent model-name prescription:
...
Workers never use High reasoning. When a task exceeds the complex-worker lane, the manager decomposes it into bounded work or reviews the difficult evidence itself.
...
- **Return:** Outcome, relevant changes, verification, exact references, and blockers.
...
Keep the manager focused on requirements, routing, task state, blockers, and acceptance:

- Require concise worker returns: outcome, relevant changes, verification, exact file, URL, task, deployment, or schema references, and unresolved blockers.
- Ask for targeted evidence such as a diff, relevant test result, filtered log excerpt, or live URL.
- Leave full logs and execution detail in the worker thread.
- Inspect raw logs, schemas, datasets, or other large evidence only when debugging or final acceptance needs it.
- Do not use global output limits, compaction tricks, or other context hacks. Scope each request and each evidence pull instead.

Token discipline must never weaken source-first routing, testing, production verification, or final review.
